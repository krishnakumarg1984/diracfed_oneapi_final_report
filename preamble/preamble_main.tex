...
\PassOptionsToPackage{luatex, paper=a4paper, hmarginratio=1:1, vmarginratio=1:1, scale=0.765}{geometry}
...
\usepackage{shellesc}
\input{preamble/generic_packages}        % general packages for a typical technical report (LuaTeX-compatible); loads the biblatex package
\input{preamble/float_related_packages}  % table/images/caption related packages (latex float handling)
\input{preamble/math_packages}           % All math-related packages particularly loads 'unicode-math' (which itself loads fontspec)
...
\input{preamble/custom_maketitle_cmd}
